        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
          ...
          <div className="lg:col-span-5 relative">
            <div className="relative mx-auto max-w-sm sm:max-w-md lg:max-w-none">
              {/* Main Fashion Featured Image - Clickable to scroll directly to products area */}
              <div
                id="hero-front-image-card"
                onClick={() => handleExplore('all')}
                role="button"
                tabIndex={0}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' || e.key === ' ') handleExplore('all');
                }}
                className="group/front cursor-pointer relative aspect-[4/5] rounded-3xl overflow-hidden shadow-2xl border-4 border-white bg-[#FAF2E9] transition-all duration-300 hover:shadow-3xl hover:border-[#D4AF37] active:scale-[0.99]"
                title={language === 'np' ? 'सामानहरू हेर्न क्लिक गर्नुहोस्' : 'Click to view product collection'}
              >
                <img
                  src="https://images.unsplash.com/photo-1610030469983-98e550d6193c?auto=format&fit=crop&w=1000&q=80"
                  alt="Dawosti Women Fashion Collection - Click to Explore Products"
                  className="w-full h-full object-cover object-top group-hover/front:scale-105 transition-transform duration-700"
                  referrerPolicy="no-referrer"
                />

                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-transparent group-hover/front:from-black/75 transition-colors" />

                {/* Direct to Products Pill Indicator */}
                <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-md px-3 py-1.5 rounded-full border border-[#D4AF37] shadow-md flex items-center gap-1.5 text-[11px] font-bold text-[#8B3A3A] group-hover/front:bg-[#8B3A3A] group-hover/front:text-white transition-all">
                  <span>{language === 'np' ? 'कलेक्सन हेर्नुहोस्' : 'Explore Collection'}</span>
                  <ArrowRight className="w-3.5 h-3.5 group-hover/front:translate-x-0.5 transition-transform" />
                </div>

                <div className="absolute bottom-4 left-4 right-4 p-4 rounded-2xl bg-white/95 backdrop-blur-md border border-[#EADCCE] shadow-lg group-hover/front:border-[#8B3A3A]/40 transition-colors">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <span className="font-bold text-[#8B3A3A] text-sm sm:text-base block">
                        {language === 'np' ? 'रु १८,९००' : 'NPR 18,900'}
                      </span>
                      <span className="text-[10px] text-[#6B564C] underline font-medium">
                        {language === 'np' ? 'क्लिक गरी सामान हेर्नुहोस् ➔' : 'Click to View ➔'}
                      </span>
                    </div>
                  </div>
                </div>
